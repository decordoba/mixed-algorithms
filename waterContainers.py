...
I encourage you to try this one out!! I spent some time working on the graphical representation of the solution
...
if __name__ == "__main__":
    ...
    t1, result1 = timeFunction(maxArea1, heights)  # Measures time taken when doing result1 = maxArea1(heights)
    t2, result2 = timeFunction(maxArea2, heights)  # Measures time taken when doing result2 = maxArea2(heights)
    ...
    water_level_y = [min(heights[result1[1]], heights[result1[2]]), min(heights[result1[1]], heights[result1[2]])]
    water_level_x = [result1[1] + 1, result1[2] + 1]
    plotLine(heights, range(1, len(heights) + 1), style="--", color="b", label="Heights (input)", show=False)
    ...
    plotText(min(heights[result1[1]], heights[result1[2]]) / 2.0, (result1[2] + result1[1] + 2) / 2.0, result1[0], fontsize="xx-large", fontweight="bold", show=True)
